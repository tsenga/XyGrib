...
MeteoWidget::MeteoWidget(QWidget *parent,GriddedPlotter* plotter) : QWidget(parent)
{
    ...
    assert(headerWidget);
    headerWidget->setObjectName("MeteoHeader");
...
void
MeteoWidget::setPlotter(GriddedPlotter *plotter)
{
    this->plotter = plotter;
}
...
void MeteoWidget::addCell_title_dataline (QString txt, bool bold,
                int lig,int col)
{
    QColor bgcolor(200,200,255); // should align with stylesheet in future

    if (Util::getSetting("showDarkSkin", true).toBool())
        bgcolor = QColor(102, 102, 102);
                ...
                )
{
    ...
    if (cellType == MTABLE_WIND_CELL) {
        bool showWindArrows = this->showWindArrows();

        cell = new TableCell_Wind (vx, vy, south, plotter,
                    this, txt, false, bgcolor, showWindArrows);
    }
    else if (cellType == MTABLE_CURRENT_CELL) {
        bool showCurrentArrows = this->showCurrentArrows();

        cell = new TableCell_Current (vx, vy, south, plotter,
                    this, txt, false, bgcolor, showCurrentArrows);
    }
    ...
    : QWidget(parent)
{
    ...
    if (Util::getSetting("showDarkSkin", true).toBool())
        this->bordercolor = QColor(204,204,204);
    else
        this->bordercolor = QColor(100,100,100); // should move align this with stylesheet in future
    ...
    : TableCell(parent, txt, bold, bgcolor)
{
    ...
    this->showWindArrows = showWindArrows; //= Util::getSetting("MTABLE_showWindArrows", true).toBool();
    ...
    : TableCell(parent, txt, bold, bgcolor)
{
    ...
    this->showCurrentArrows = showCurrentArrows; // Util::getSetting("MTABLE_showCurrentArrows", true).toBool();
